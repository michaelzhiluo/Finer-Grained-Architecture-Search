...
class GeneralizedConvNetwork(object):
	...
	def build_model(self): 
		...
		self.build_weight_graph()
		if self.has_hypers:
			self.build_hyper_graph()
		
	def build_weight_graph(self):
		self.output = self.feed_forward(self.train_input_reshape, self.weights, self.config["model"], reuse = False)
	...
	def build_hyper_graph(self):
		...
		self.output1 = self.feed_forward(self.test_input_reshape, look_ahead_weights, self.config["model"], reuse = True)
	...
	def generate_placeholders(self):
		...
		self.train_input = tf.placeholder(tf.float32, [None, input_dim])
		self.train_input_reshape = tf.reshape(self.train_input, input_shape)
		self.train_label = tf.placeholder(tf.float32, [None, 10]) 
		self.test_input = tf.placeholder(tf.float32, [None, input_dim])
		self.test_input_reshape = tf.reshape(self.test_input, input_shape) 
		...
		with tf.variable_scope("Weights", reuse = tf.AUTO_REUSE):
			for i in range(0, len(layer_list)):
				...
				if layer_dict["type"]=="FC":
					...
					self.has_hypers = True if layer_dict["type"] == "Dense" else self.has_hypers
					self.weights["w" + str(counter)] = tf.get_variable("w" + str(counter), 
	...
	def feed_forward(self, inp, weights, model_config, reuse=True):
		layer_list = model_config["layers"]
		counter =0
		output = inp
		print(output)
		for i in range(0, len(layer_list)):
			...
			if layer_dict["type"]=="FC":
				...
				pool_config = layer_dict["pool"] if "pool" in layer_dict else None
				output = DenseLayer(output, layer_dict["output_dim"] , weights["w"+str(counter)], weights["b"+str(counter)], self.train_weights, self.exploration, activation_fn, norm_type, pool_config, reuse = reuse)
				counter+=1
			print("hi")
			print(output)
		return output